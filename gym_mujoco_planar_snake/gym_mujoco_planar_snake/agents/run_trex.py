...
class Model(object):
    ...
    def train(self,D,batch_size=64,iter=10000,l2_reg=0.01,noise_level=0.1,debug=False):
        ...
        for it in tqdm(range(iter),dynamic_ncols=True):
            ...
            loss,l2_loss,acc,_good = sess.run([self.loss,self.l2_loss,self.acc,self.update_op],feed_dict={
                self.x:b_x,
                self.y:b_y,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--preference_type', default='gt', help='gt or gt_traj or time or gt_traj_no_steps, gt_traj_no_steps_n_mix; if gt then preference will be given as a GT reward, otherwise, it is given as a time index')
    ...
    if not args.eval :
        ...
        template = 'python3 -m baselines.run --alg=ppo2 --env={env} --num_timesteps=1e6 --save_interval=10 --custom_reward {custom_reward} --custom_reward_kwargs="{kwargs}" --gamma {gamma}'
        kwargs = {
        ...
        pposgd_simple.learn(enargs.env_idv, policy_fn,
